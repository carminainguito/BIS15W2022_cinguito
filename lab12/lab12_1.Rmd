...
inner_exampleDF #similar data is shared 
```
...
full_exampleDF #contains NAs, that's okay
```
...
```{r}
spiders_all <- bind_rows(spiders_with_locs)
```
...
```{r}
write.csv(spiders_with_locs, file = "spiders_with_locs.csv", row.names = FALSE) #don't need numerical values
```
