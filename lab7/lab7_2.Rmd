...
```{r}
naniar::miss_var_summary(life_history) #similar to purrr, but adds % missing
```
...
```{r}
naniar::miss_var_summary(cites) #how NAs are distributed 
```
...
```{r}
vis_dat(life_history) #classes of data, overall distribution 
```
