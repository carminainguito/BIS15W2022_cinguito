...
variables- columns while observations are seen as rows
...
## `select()`
Select allows you to pull out columns (variables) of interest from a dataframe. To do this, just add the names of the columns to the `select()` command. The order in which you add them, will determine the order in which they appear in the output.
```{r}
names(fish)
```
...
```{r}
mammals_new <- rename(mammals,genus="Genus", mean_mass="wean mass", max_life="max. life", litter_size="litter size", litters_per_year="litters/year")
mammals_new
```
