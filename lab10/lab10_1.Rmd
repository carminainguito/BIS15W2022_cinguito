...
```{r message=FALSE, warning=FALSE}
life_history <- read_csv(here("lab10", "data", "mammal_lifehistories_v2.csv")) %>% na_if("-999") %>% clean_names() #don't do this when you have not used the data before
```
...
life_history %>% 
  group_by(order) %>% 
  summarize(mean_mass=mean(mass, na.rm=T)) #ggplot would put these large numbers in scientific notation 
```
...
```{r}
life_history %>% 
  ggplot(aes(x=gestation, y=wean_mass)) + geom_point(na.rm=T) + scale_y_log10() + geom_smooth(method="lm") #removing scale y would cause a problem
```
...
life_history %>% 
  ...
  geom_boxplot()+scale_y_log10()+ #account for huge differences in mass
  coord_flip()
```
...
```{r}
elephants %>%
  ggplot(aes(x=sex(order,mean_mass), y=mean_mass))+
  geom_bar()+coord_flip()+
  scale_y_log10() 
```
...
elephants %>% 
  ...
  labs(title = "Elephant Age vs. Height",
       x = "Age",
       y = "Height") +
  theme(plot.title = element_text(size = rel(1.5), hjust = 0.5)) #relative size to x and y axis, basically centering or moving the title 
```
...
```{r}
elephants %>% 
  ggplot(aes(x=sex, fill=sex))+geom_bar()+ #fill adds color by variable sex
...
homerange %>%
  ...
  labs(title = "Number of Ectotherms and Endotherms",
       x = "Thermoregulation",
       y = "n") +
  theme(plot.title = element_text(size = rel(1.30), hjust = 0.5))
```
...
homerange %>%
  ...
  geom_point(size=1.5, color="red", alpha0.5)+
