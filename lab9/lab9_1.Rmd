...
ggplot(data=homerange, mapping=aes(x=log10.preymass, y=log10.hra)) +
  geom_point(na.rm=T) #by default gg plot is going to remove the NAs 
```

### Bar Plot: `geom_bar()`
The simplest way of bar plot counts the number of observations in a categorical variable. In this case, we want to know how many observations are present in the variable `trophic.guild`. Notice that we do not specify a y-axis because it is count by default.  
...
```{r}
homerange %>% 
  count(trophic.guild) #group by is the same way to do this (then summary), given categorical and count
```
