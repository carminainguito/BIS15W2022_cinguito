...
```{r}
fish %>% #calling data and feeding through functions you want to use
...
fish %>% 
  select(lakeid, scalelength) %>% 
  arrange(scalelength) #this by itself would give ascending order
```
...
fish %>%
  ...
  select(fish_id, length, length_half) #mutate(new column name)
```
