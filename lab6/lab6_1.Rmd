...
```{r}
msleep %>% 
  filter(!is.na(vore)) %>% #note that question is asking for taxonomic orders
  tabyl(vore, order)
```
...
```{r}
head(msleep) #view sleep_total variable
```
...
msleep %>% 
  ...
  summarize(mean_sleep_lg = mean(sleep_total)) #give new name of variable 
```
...
msleep %>%
  ...
  summarise(mean_bodywt=mean(bodywt),
            ...
            n_samples=n()) #provide total number of observations
```
