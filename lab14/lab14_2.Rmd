...
flying_vec <- homerange %>%
  filter(locomotion == "flying")%>% #create a vector for all rows 
  pull(common.name)
flying_vec
```
...
Woah! an image! Swim and fly are mutually exclusive , utilize different columns of data
...
practice_venn <- list(ecto_vec, aqua_vec, carni_vec, swimming_vec)

# Make the plot:
ggVennDiagram(practice_venn, category.names = c("Swimming", "Carnivores", "Aquatic", "Ectotherms"), edge_size = 1.5)

venn_plot +
  scale_fill_distiller(palette = "Greys")+ #distiller is for discrete data
  labs(title = "Intersection Between 4 Categories ")
```
